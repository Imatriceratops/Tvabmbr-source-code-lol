...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Never invited again', 0.2], //From 0% to 19%
		['OK this is bad', 0.4], //From 20% to 39%
		['40% not that bad', 0.5], //From 40% to 49%
		['Youre getting there', 0.6], //From 50% to 59%
		['Close', 0.69], //From 60% to 68%
		['No just no', 0.7], //69%
		['Closer', 0.8], //From 70% to 79%
		['Almost there', 0.9], //From 80% to 89%
		['OMGZ! YOU ALMOST PASSED JUST KEEP GOING!!', 1], //From 90% to 99%
		['Congrats U did it now.. Keep the FC for the whole song', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		if(SONG.stage.toLowerCase() == 'comasroom') {
       	introSoundsSuffix = '-starjai';	
		 }
	...
	public dynamic function fullComboFunction()
	{
		...
		if(songMisses == 0)
		{
			if (bads > 0 || shits > 0) ratingFC = 'Rockin FC!';
			else if (goods > 0) ratingFC = 'Electric FC!!';
			else if (sicks > 0) ratingFC = 'Super FC!!!';
		}
		else {
			if (songMisses < 10) ratingFC = 'UHHHH';
			else ratingFC = 'Clear';
		}
	}
